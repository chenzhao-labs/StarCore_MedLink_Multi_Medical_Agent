...
# Initialize SQLite checkpoint storage (persists across restarts)
...
def create_agent_graph():
    ...
    # Alias to avoid shadowing with RunnableConfig parameter in nodes
    app_config = config
    ...
    def analyze_input(state: AgentState) -> AgentState:
        ...
        if input_text:
            ...
            if not is_allowed:
                ...
                logger.info("Selected agent: INPUT GUARDRAILS, Message: %s", message)
                return {
                    **state,
                    "messages": message,
        ...
        if isinstance(current_input, dict) and "image" in current_input:
            ...
            logger.info("ANALYZED IMAGE TYPE: %s", image_type)
    ...
    def route_to_agent(state: AgentState) -> Dict:
        ...
        # Decided agent
        logger.info("Decision: %s", decision['agent'])
    ...
    def run_conversation_agent(state: AgentState, config: RunnableConfig = None) -> AgentState:
        ...
        **You:** "I'm not a doctor, but headaches and fever can have various causes, from infections to dehydration. If your symptoms persist, you should see a medical professional."
        ...
        if token_queue:
            ...
            for chunk in app_config.conversation.llm.stream(
                conversation_prompt, config={"callbacks": [callback]}
            ):
                chunks.append(chunk)
            full_text = "".join(c.content for c in chunks if c.content)
            response = AIMessage(content=full_text)
        else:
            response = app_config.conversation.llm.invoke(conversation_prompt)
    ...
    def run_rag_agent(state: AgentState, config: RunnableConfig = None) -> AgentState:
        ...
        response = rag_agent.process_query(
            query, chat_history=recent_context, token_queue=token_queue
        )
        retrieval_confidence = response.get("confidence", 0.0)  # Default to 0.0 if not provided

        logger.info("Retrieval Confidence: %s", retrieval_confidence)
        ...
        logger.debug("Response text type: %s", type(response_text))
        logger.debug("Response text preview: %s...", response_text[:100])
        ...
        if isinstance(response_text, str) and (
            ...
            logger.debug("Response text that triggered insufficient_info: %s...", response_text[:100])
    ...
    def run_web_search_processor_agent(state: AgentState, config: RunnableConfig = None) -> AgentState:
        ...
        messages = state["messages"]
        web_search_context_limit = app_config.web_search.context_limit
    ...
    def confidence_based_routing(state: AgentState) -> Dict[str, str]:
        ...
        # Debug prints
        logger.debug("Routing check - Retrieval confidence: %s", state.get('retrieval_confidence', 0.0))
        logger.debug("Routing check - Insufficient info flag: %s", state.get('insufficient_info', False))
        
        # Redirect if confidence is low or if response indicates insufficient info
        if (state.get("retrieval_confidence", 0.0) < app_config.rag.min_retrieval_confidence or
...
def process_query(
    ...
    Args:
        ...
        token_queue: Optional TokenQueue for streaming tokens
        thread_id: Unique thread identifier for conversation isolation
    ...
    try:
        ...
        pass  # Windows GBK console can't render emoji

    return result


async def process_query_stream(query: Union[str, Dict], thread_id: str = "1") -> dict:
    """
    Async generator: yields (event_type, data) tuples for SSE streaming.
    event_type: "token", "agent", "done", "error"
    """
    token_queue = TokenQueue()

    def _run():
        try:
            result = process_query(query, token_queue=token_queue, thread_id=thread_id)
            token_queue._result = result  # stash for later
        except Exception as e:
            token_queue.error(str(e))
        finally:
            token_queue.mark_done()
    ...
    async for token in token_queue:
        if token.startswith("__ERROR__:"):
            yield ("error", token[len("__ERROR__:"):])
            return
        yield ("token", token)
        if token_queue._started and not seen_agent:
            # We won't know the agent until process_query completes,
            # signal the frontend that tokens have started
            pass

    thread.join(timeout=30)

    result = getattr(token_queue, "_result", None)
    if result:
        agent_name = result.get("agent_name", "UNKNOWN")
        yield ("agent", agent_name)
        yield ("done", result["messages"][-1].content if result.get("messages") else "")
    else:
