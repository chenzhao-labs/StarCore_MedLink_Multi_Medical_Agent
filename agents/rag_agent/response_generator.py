    ...
    def generate_response(
        ...
        try:
            ...
            if token_queue:
                ...
                chunks = []
                for chunk in self.response_generator_model.stream(
                    prompt, config={"callbacks": [callback]}
                ):
                    chunks.append(chunk)
                response_text = "".join(c.content for c in chunks if c.content)
            else:
                response_text = self.response_generator_model.invoke(prompt).content
